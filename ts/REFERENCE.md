# UseragentParser TypeScript SDK Reference

Complete API reference for the UseragentParser TypeScript SDK.
...
```ts
new UseragentParserSDK(options?: object)
```
...
#### `UseragentParserSDK.test(testopts?, sdkopts?)`
...
**Returns:** `UseragentParserSDK` instance in test mode.
...
Create a new `Parse` entity instance.
...
Alias for `UseragentParserSDK.test()`.
...
```ts
const result = await client.Parse().load({ key: 'key', ua: 'ua' })
```
...
Return the parent `UseragentParserSDK` instance.
