...
```ts
try {
  const parse = await client.Parse().load({ key: 'example_key', ua: 'example_ua' })
  console.log(parse)
} catch (err) {
...
```ts
try {
  const parse = await client.Parse().load({ key: "example", ua: "example" })
  console.log(parse)
} catch (err) {
  console.error('load failed:', err)
}
...
await entity.load({ key: 'example_key', ua: 'example_ua' })
...
```ts
const parse = await client.Parse().load({ key: 'key', ua: 'ua' })
```
...
await parse.load({ key: "example", ua: "example" })
