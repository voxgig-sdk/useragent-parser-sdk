...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.USERAGENTPARSER_TEST_LIVE ||
    'TRUE' === process.env.USERAGENTPARSER_TEST_OVERRIDE
  ) {
  ...
  m.USERAGENTPARSER_TEST_EXPLAIN = process.env.USERAGENTPARSER_TEST_EXPLAIN || m.USERAGENTPARSER_TEST_EXPLAIN
