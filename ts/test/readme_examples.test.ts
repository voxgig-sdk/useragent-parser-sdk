...
//     (\`new UseragentParserSDK(...)\` / \`UseragentParserSDK.test(...)\` become
//     \`UseragentParserSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'UseragentParserSDK'
...
const TEST_SEED = {"entity":{"parse":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new UseragentParserSDK(...)\` and any
// \`UseragentParserSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+UseragentParserSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/UseragentParserSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
