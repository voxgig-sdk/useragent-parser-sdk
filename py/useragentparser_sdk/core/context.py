...
from useragentparser_sdk.core.helpers import get_ctx_prop, to_map
...
class UseragentParserContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = UseragentParserControl()
        ...
        if isinstance(sp, UseragentParserSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, UseragentParserResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, UseragentParserResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return UseragentParserOperation({})
    ...
    def make_error(self, code, msg):
        return UseragentParserError(code, msg, self)
