...
The Python SDK for the UseragentParser API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Parse(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.UseragentParser(None).load(
    {"id": "test01"}, None
)
