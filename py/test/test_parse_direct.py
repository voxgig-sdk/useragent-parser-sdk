...
class TestParseDirect:
    ...
    def test_should_direct_load_parse(self):
        setup = _parse_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-parse", "live" if setup["live"] else "unit")
        if _skip:
...
def _parse_direct_setup(mockres):
