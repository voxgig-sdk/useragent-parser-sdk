# UseragentParser SDK test runner
...
class UseragentParserTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            UseragentParserTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = UseragentParserTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = UseragentParserTestRunner.getenv("USERAGENT_PARSER_TEST_LIVE")
        override = UseragentParserTestRunner.getenv("USERAGENT_PARSER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = UseragentParserTestRunner.getenv(key)
        ...
        explain = UseragentParserTestRunner.getenv("USERAGENT_PARSER_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if UseragentParserTestRunner._test_control is not None:
            return UseragentParserTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                UseragentParserTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            UseragentParserTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = UseragentParserTestRunner.load_test_control()
...
def load_env_local():
    UseragentParserTestRunner.load_env_local()


def env_override(m):
    return UseragentParserTestRunner.env_override(m)


def entity_data(v):
    return UseragentParserTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return UseragentParserTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return UseragentParserTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return UseragentParserTestRunner.load_test_control()


def live_client_options():
    return UseragentParserTestRunner.live_client_options()


def live_delay_ms():
    return UseragentParserTestRunner.live_delay_ms()
