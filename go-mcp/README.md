...
[MCP](https://modelcontextprotocol.io) server exposing the UseragentParser SDK as
two agent tools — `useragent-parser_list` and `useragent-parser_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/useragent-parser-mcp)
make build

# 2. Provide credentials via the environment
export USERAGENT_PARSER_APIKEY=sk_live_xxx
...
```jsonc
// useragent-parser_list: first page of records
{ "entity": "parse" }
{ "entity": "parse", "query": { } }

// useragent-parser_load: one record by id
{ "entity": "parse", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/useragent-parser-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user useragent-parser \
     -- "$PWD"/dist/*/useragent-parser-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `useragent-parser_list` and `useragent-parser_load` tools now appear
   in new sessions. Ask the agent to *"list parse using useragent-parser"*
   and it calls `useragent-parser_list` with `{"entity":"parse"}`.
...
```sh
export USERAGENT_PARSER_APIKEY=sk_live_xxx            # API key
export USERAGENT_PARSER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `useragent-parser_list` tool
...
### Call the `useragent-parser_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `useragent-parser_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `useragent-parser_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `USERAGENT_PARSER_APIKEY` | API key sent with every request. |
| `USERAGENT_PARSER_BASE` | Optional override of the API base URL. |
