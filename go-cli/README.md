...
boru-driven command-line client **and** interactive REPL for the UseragentParser
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/useragent-parser-cli)
make build

# 2. See usage (words, entities, env vars)
./useragent-parser-cli --help

# 3. Provide credentials once, via the environment
export USERAGENT_PARSER_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./useragent-parser-cli load 1 parse            # {id:1} shorthand
./useragent-parser-cli load '{id:1}' parse       # explicit match map

# 5. Override the API base URL for a single call
USERAGENT_PARSER_BASE=https://api.example.com ./useragent-parser-cli load 1 parse

# 6. No arguments -> interactive REPL
./useragent-parser-cli
useragent-parser> load 1 parse
useragent-parser> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/useragent-parser-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/useragent-parser-cli load 1 parse
   ```
...
```sh
./useragent-parser-cli load 1 parse          # scalar shorthand for {id:1}
./useragent-parser-cli load '{id:1}' parse     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `useragent-parser>`). Each line is
evaluated as its own boru expression:

```text
$ ./useragent-parser-cli
useragent-parser> load 1 parse
useragent-parser> /help
useragent-parser> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `parse`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/useragent-parser-cli`. |
...
(`./useragent-parser-cli <expr>`) and interactively (the REPL), and expressions compose the
