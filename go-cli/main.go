// Command UseragentParser-cli is a CLI + REPL for the UseragentParser SDK,
...
const prompt = "useragent-parser"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "parse"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      USERAGENT_PARSER_APIKEY (api key), USERAGENT_PARSER_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: USERAGENT_PARSER_APIKEY carries the API key and
	// USERAGENT_PARSER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("USERAGENT_PARSER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("USERAGENT_PARSER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewUseragentParserSDK(opts)
