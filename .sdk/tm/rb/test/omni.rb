...
module UseragentParserOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if UseragentParserOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = UseragentParserOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| UseragentParserOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || UseragentParserOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = UseragentParserSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = UseragentParserError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { UseragentParserOmni.errify(err) }
