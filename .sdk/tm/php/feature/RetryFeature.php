...
// UseragentParser SDK retry feature
...
class UseragentParserRetryFeature extends UseragentParserBaseFeature
{
    ...
    public function init(UseragentParserContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (UseragentParserContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(UseragentParserContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(UseragentParserContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
