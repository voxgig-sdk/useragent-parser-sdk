# UseragentParser Golang SDK Reference

Complete API reference for the UseragentParser Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *UseragentParserSDK`
...
#### `Parse(data map[string]any) UseragentParserEntity`

Create a new `Parse` entity instance. Pass `nil` for no initial data.
...
```go
result, err := client.Parse(nil).Load(map[string]any{"key": "key", "ua": "ua"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
