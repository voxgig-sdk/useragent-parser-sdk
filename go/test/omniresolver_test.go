...
import (
	...
	omni "github.com/voxgig-sdk/useragent-parser-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.UseragentParserError); is {
				out := map[string]any{
					"name":    "UseragentParserError",
...
func omniCtx(arg any, client *sdk.UseragentParserSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.UseragentParserSDK); is {
			client = live
