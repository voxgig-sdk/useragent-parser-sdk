...
module UseragentParserUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = UseragentParserResponse.new({})
    ...
    if fetch_err
      ...
      response = UseragentParserResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = UseragentParserResponse.new(fetched)
    else
