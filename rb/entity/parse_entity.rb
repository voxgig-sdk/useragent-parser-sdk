...
class ParseEntity
  ...
  def make
    opts = @_entopts.dup
    ParseEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = UseragentParserHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Parse, Hash] the current Parse data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = UseragentParserHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Parse fields)
  def match_get
  ...
  # Load a single Parse.
  #
  # @param reqmatch [ParseLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Parse.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Parse, Hash] the loaded Parse; raises UseragentParserError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = UseragentParserHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
