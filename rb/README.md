# UseragentParser Ruby SDK



The Ruby SDK for the UseragentParser API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
begin
  # load returns the ENTITY — call data_get for the Parse record (raises on error).
...
raise a `UseragentParserError` (a `StandardError` subclass), so wrap
...
```
rb/
├── UseragentParser_sdk.rb       -- Main SDK module
