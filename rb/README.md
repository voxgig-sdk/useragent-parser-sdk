...
The Ruby SDK for the UseragentParser API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Parse(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.UseragentParser(nil).load(
  { "id" => "test01" }, nil
)
