...
require_relative 'UseragentParser_types'
...
class UseragentParserSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = UseragentParserHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = UseragentParserHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, UseragentParserFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    UseragentParserUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = UseragentParserHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = UseragentParserHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = UseragentParserHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue UseragentParserError => err
    ...
    if fetched.is_a?(Hash)
      status = UseragentParserHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = UseragentParserError.new(
        "graphql_error", "UseragentParserSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = UseragentParserSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
