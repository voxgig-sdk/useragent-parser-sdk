...
// UseragentParser SDK feature test
//
...
class FtCtrl extends UseragentParserControl
{
...
class FtClient
{
    ...
    private UseragentParserUtility $utility;

    public function __construct(UseragentParserUtility $utility, array $options)
    {
    ...
    public function get_utility(): UseragentParserUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (UseragentParserContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public UseragentParserContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (UseragentParserContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = UseragentParserConfig::make_feature($name);
    ...
    private static function buildUrl(UseragentParserSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof UseragentParserError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new UseragentParserResponse($fetched);
        }
    ...
    private function fail(UseragentParserContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (UseragentParserContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
