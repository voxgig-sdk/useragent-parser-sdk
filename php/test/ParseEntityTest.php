...
class ParseEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        ...
        $ent = $testsdk->Parse(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = parse_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "parse." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set USERAGENT_PARSER_TEST_PARSE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $parse_ref01_data = null;
        if (count($parse_ref01_data_raw) > 0) {
...
function parse_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/parse/ParseTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["parse01", "parse02", "parse03"] as $k) {
    ...
    $entid_env_raw = getenv("USERAGENT_PARSER_TEST_PARSE_ENTID");
    ...
    if ($env["USERAGENT_PARSER_TEST_LIVE"] === "TRUE") {
        ...
        $client = new UseragentParserSDK(Helpers::to_map($merged_opts));
    }
