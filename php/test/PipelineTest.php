...
// UseragentParser SDK pipeline test
//
...
{
    private static function utility(): UseragentParserUtility
    {
        return new UseragentParserUtility();
    }
    ...
    private static function ctx(array $over = []): UseragentParserContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new UseragentParserContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new UseragentParserOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof UseragentParserError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new UseragentParserOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = UseragentParserMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new UseragentParserOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = UseragentParserMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new UseragentParserOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = UseragentParserMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new UseragentParserSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = UseragentParserMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new UseragentParserResponse([]),
            'result' => new UseragentParserResult([]),
        ]);
        [, $err] = UseragentParserMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new UseragentParserSpec([]),
            'response' => null,
            'result' => new UseragentParserResult([]),
        ]);
        [, $err] = UseragentParserMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new UseragentParserSpec(['step' => 's']),
            'response' => new UseragentParserResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new UseragentParserResult([]),
        ]);
        [, $err] = UseragentParserMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new UseragentParserSpec(['step' => 's']),
            'response' => new UseragentParserResponse(self::resp(200, ['v' => 1])),
            'result' => new UseragentParserResult([]),
        ]);
        [, $err] = UseragentParserMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new UseragentParserSpec(['step' => 's']),
            'response' => new UseragentParserResponse(self::resp(200, ['v' => 2])),
            'result' => new UseragentParserResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        UseragentParserMakeResponse::call($ctx);
    ...
    {
        $preset = new UseragentParserResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new UseragentParserSpec([]),
            'response' => new UseragentParserResponse([]),
            'result' => new UseragentParserResult([]),
        ...
        [$got, $err] = UseragentParserMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new UseragentParserResult([])]);
        [, $err] = UseragentParserMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new UseragentParserSpec([]), 'result' => null]);
        [, $err] = UseragentParserMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new UseragentParserSpec(['step' => 's']),
            'result' => new UseragentParserResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new UseragentParserOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = UseragentParserMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new UseragentParserSpec(['step' => 's']),
            'result' => new UseragentParserResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new UseragentParserOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = UseragentParserMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new UseragentParserResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new UseragentParserSpec([]),
            'result' => new UseragentParserResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = UseragentParserMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = UseragentParserMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new UseragentParserError('boom', 'boom');
        $utility->fetcher = function (UseragentParserContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new UseragentParserSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = UseragentParserMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (UseragentParserContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = UseragentParserMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(UseragentParserResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        UseragentParserMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (UseragentParserContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new UseragentParserSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = UseragentParserMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new UseragentParserResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new UseragentParserSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = UseragentParserMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = UseragentParserMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = UseragentParserMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new UseragentParserResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, UseragentParserDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new UseragentParserResult(['ok' => false])]);
        $this->expectException(UseragentParserError::class);
        UseragentParserDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new UseragentParserResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', UseragentParserMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        UseragentParserMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new UseragentParserBaseFeature();
        $b = new UseragentParserBaseFeature();
        UseragentParserFeatureAdd::call($ctx, $a);
        UseragentParserFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): UseragentParserBaseFeature
    {
        $f = new UseragentParserBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        UseragentParserFeatureAdd::call($ctx, self::named_feature('a'));
        UseragentParserFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        UseragentParserFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        UseragentParserFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        UseragentParserFeatureAdd::call($ctx, $replace);
        ...
        UseragentParserFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): UseragentParserContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new UseragentParserSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = UseragentParserPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        UseragentParserPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new UseragentParserResponse(['headers' => null]),
            'result' => new UseragentParserResult([]),
        ]);
        UseragentParserResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        UseragentParserResultBody::call($ctx);
