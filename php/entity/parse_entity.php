...
// UseragentParser SDK Parse entity
...
class ParseEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ParseEntity($this->_client, $opts);
    }

    /**
     * @param Parse|array $args Parse data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = UseragentParserHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Parse|array The current Parse data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Parse fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = UseragentParserHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Parse fields).
     */
     ...
     * Load a single Parse.
     *
     * @param ParseLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ParseLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Parse|array The loaded Parse as an assoc-array at the
     *   SDK boundary; throws UseragentParserError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = UseragentParserHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
