...
class UseragentParserSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = UseragentParserHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = UseragentParserHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!UseragentParserFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, UseragentParserFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return UseragentParserUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = UseragentParserHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = UseragentParserHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = UseragentParserHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new UseragentParserError($op . "_allow",
                "UseragentParserSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = UseragentParserHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new UseragentParserError("graphql_error",
                "UseragentParserSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Parse($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_parse === null) {
                $this->_parse = new ParseEntity($this, null);
            }
            return $this->_parse;
        }
        return new ParseEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new UseragentParserSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
