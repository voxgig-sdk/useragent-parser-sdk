...
class UseragentParserMakeError
{
    public static function call(?UseragentParserContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new UseragentParserContext([], null);
        }
        $op = $ctx->op ?? new UseragentParserOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new UseragentParserResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof UseragentParserError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new UseragentParserError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof UseragentParserError) {
