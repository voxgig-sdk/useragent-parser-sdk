# UseragentParser Lua SDK



The Lua SDK for the UseragentParser API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Parse()` — each with the same small set of operations (`load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/useragent-parser-sdk/releases)),
...
```lua
local parse, err = client:Parse():load({ key = "example_key", ua = "example_ua" })
...
```lua
local parse, err = client:Parse():load({ key = "example", ua = "example" })
if err then error(err) end
```
...
local result, err = client:Parse():load({ key = "example", ua = "example" })
...
Check `err` first (it is non-`nil` on failure), then use `value`:
    ...
    local parse, err = client:Parse():load()
    if err then error(err) end
...
```lua
local parse, err = client:Parse():load({ key = "key", ua = "ua" })
```
...
```
lua/
├── useragent-parser_sdk.lua    -- Main SDK module
...
The main module (`useragent-parser_sdk`) exports the SDK constructor
