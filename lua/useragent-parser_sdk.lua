...
require("useragent-parser_types")
...
local UseragentParserSDK = {}
UseragentParserSDK.__index = UseragentParserSDK
...
UseragentParserSDK._make_feature = _make_feature


function UseragentParserSDK.new(options)
  local self = setmetatable({}, UseragentParserSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function UseragentParserSDK:options_map()
...
function UseragentParserSDK:get_utility()
  return Utility.copy(self._utility)
end


function UseragentParserSDK:get_root_ctx()
  return self._rootctx
end


function UseragentParserSDK:prepare(fetchargs)
...
function UseragentParserSDK:direct(fetchargs)
...
function UseragentParserSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function UseragentParserSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "UseragentParserSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function UseragentParserSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function UseragentParserSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "UseragentParserSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function UseragentParserSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = UseragentParserSDK.new(sdkopts)
  sdk.mode = "test"
