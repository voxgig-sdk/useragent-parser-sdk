...
local ParseEntity = {}
ParseEntity.__index = ParseEntity


function ParseEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ParseEntity)
  self._name = "parse"
  self._client = client
...
function ParseEntity:get_name()
  return self._name
end


function ParseEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ParseEntity.new(self._client, opts)
end
...
function ParseEntity:mark_deleted()
  self._deleted = true
end


function ParseEntity:deleted()
  return true == self._deleted
end


function ParseEntity:data_set(args)
  if args ~= nil then
...
function ParseEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ParseEntity:match_set(args)
  if args ~= nil then
...
function ParseEntity:match_get()
...
function ParseEntity:stream(action, args, callopts)
...
---@param reqmatch ParseLoadMatch
---@param ctrl? table
---@return Parse
---@return string? err
function ParseEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ParseEntity:_run_op(ctx, post_done)
  local utility = self._utility
